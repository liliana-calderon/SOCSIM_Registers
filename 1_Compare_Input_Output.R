...
asfr_hetfert0 <- estimate_fertility_rates(opop = opop_hetfert0,
                                          ...
                                          age_group = 5) # [,)
save(asfr_hetfert0, file = "Measures/asfr_hetfert0.RData")

# Retrieve age-specific mortality rates with hetfert 0
asmr_hetfert0 <- estimate_mortality_rates(opop = opop_hetfert0,
                                          final_sim_year = 2022, #[Jan-Dec]
                                          year_min = 1750, # Closed
                                          year_max = 2020, # Open )
                                          year_group = 5,
                                          ...
                                          age_group = 5) # [,) 
save(asmr_hetfert0, file = "Measures/asmr_hetfert0.RData")


## Plots ASFR and ASMR

# Load ASFR and ASMR from the simulations 
load("Measures/asfr_hetfert0.RData")
load("Measures/asmr_hetfert0.RData")
...
age_levels <- levels(asmr_hetfert0$age)

bind_rows(asfr_hetfert0 %>%
            mutate(rate = "ASFR",                   
                   sex = "female"),
          asmr_hetfert0 %>% 
            mutate(rate = "ASMR") %>% 
...
ggsave(file="Graphs/SOCSIM_ASFR_ASMR_hetfert0.jpeg", width=17, height=9, dpi=300)
...
year_breaks_fert <- unique(as.numeric(str_extract_all(asfr_hetfert0$year, "\\d+", simplify = T)))
...
age_breaks_fert <- unique(as.numeric(str_extract_all(asfr_hetfert0$age, "\\d+", simplify = T)))
...
bind_rows(HFCD0, SocsimF0) %>%
  ...
  scale_x_discrete(guide = guide_axis(angle = 90)) +
  theme_graphs()
ggsave(file="Graphs/HFD_SOCSIM_ASFR_hetfert0.jpeg", width=17, height=9, dpi=300)
...
# Year breaks. Extract all the unique numbers from the intervals 
year_breaks_mort <- unique(as.numeric(str_extract_all(asmr_hetfert0$year, "\\d+", simplify = T)))
...
age_breaks_mort <- unique(as.numeric(str_extract_all(asmr_hetfert0$age, "\\d+", simplify = T)))
...
bind_rows(HMD, SocsimM) %>% 
  ...
  scale_x_discrete(guide = guide_axis(angle = 90)) +
  theme_graphs()
ggsave(file="Graphs/HMD_SOCSIM_ASMR_hetfert0.jpeg", width=17, height=9, dpi=300)
...
ggsave(file="Graphs/Socsim_HFD_HMD1_hetfert0.jpeg", width=17, height=9, dpi=300)
#----------------------------------------------------------------------------------------------------
...
asfr_hetfert0_1 <- estimate_fertility_rates(opop = opop_hetfert0,
                                            final_sim_year = 2022 , #[Jan-Dec]
                                            year_min = 1750, # Closed [
                                            year_max = 2023, # Open )
                                            year_group = 1, 
                                            ...
                                            age_max_fert = 55, # Open )
                                            age_group = 1) # [,)
save(asfr_hetfert0_1, file = "Measures/asfr_hetfert0_1.RData")

# Load ASFR 1x1 and calculate TFR for plotting ----
load("Measures/asfr_hetfert0_1.RData")

# Year breaks. Extract all the unique numbers from the intervals. 
year_breaks_fert_1 <- unique(as.numeric(str_extract_all(asfr_hetfert0_1$year, "\\d+", simplify = T)))

# Year range to filter HFD data
year_range_fert_1 <- min(year_breaks_fert_1):max(year_breaks_fert_1-1)

# Age breaks of fertility rates. Extract all the unique numbers from the intervals 
age_breaks_fert_1 <- unique(as.numeric(str_extract_all(asfr_hetfert0_1$age, "\\d+", simplify = T)))
...
bind_rows(TFR_HFCD, TFR_hetfert0) %>%
  ...
  scale_x_discrete(guide = guide_axis(angle = 90)) +
  theme_graphs()
ggsave(file="Graphs/HFD_SOCSIM_TFR_hetfert0.jpeg", width=17, height=9, dpi=300)

# Summary measure of error in TFR ----

# Differences of means
DM_TFR <- bind_rows(TFR_HFCD, TFR_hetfert0) %>%
  filter(Year > 1750) %>% 
...
# Mean of differences
MD_TFR <- bind_rows(TFR_HFCD, TFR_hetfert0) %>% 
  filter(Year > 1750) %>% 
  pivot_wider(id_cols = Year, names_from = "Source", values_from = "TFR") %>% 
  mutate(Error = SOCSIM - `HFC/HFD`) %>% 
  group_by(Year) %>% 
...
bind_rows(DM_TFR, MD_TFR) %>%
  ...
  theme_graphs()
# ggsave(file="Graphs/HFD_SOCSIM_TFR_Error_hetfert0.jpeg", width=17, height=9, dpi=300)
...
asmr_hetfert0_1 <- estimate_mortality_rates(opop = opop_hetfert0,
                                            final_sim_year = 2022, #[Jan-Dec]
                                            year_min = 1750, # Closed
                                            year_max = 2023, # Open )
                                            year_group = 1,
                                            age_max_mort = 110, # Open )
                                            age_group = 1) # [,)
save(asmr_hetfert0_1, file = "Measures/asmr_hetfert0_1.RData")

# Compute life tables from asmr 1x1 from the SOCSIM simulation
lt_hetfert0 <- lt_socsim(asmr_socsim = asmr_hetfert0_1)
save(lt_hetfert0, file = "Measures/lt_hetfert0.RData")


# Load and wrangle life tables for plotting ----

# Load life tables from asmr 1x1 from the SOCSIM simulation
load("Measures/lt_hetfert0.RData")
...
load("Measures/asmr_hetfert0_1.RData")

# Year breaks. Extract all the unique numbers from the intervals 
year_breaks_mort_1 <- unique(as.numeric(str_extract_all(asmr_hetfert0_1$year, "\\d+", simplify = T)))
...
lt_hetfert0b <- lt_hetfert0 %>%
  mutate(Year = as.numeric(str_extract(year, "\\d+")),
         Source = "SOCSIM") %>% 
  select(Year, ex, Source, sex, Age)

bind_rows(lt_HMD, lt_hetfert0b) %>% 
  ...
  theme_graphs()+
  labs(y = "e0") 
ggsave(file="Graphs/HMD_SOCSIM_e0_hetfert0.jpeg", width=17, height=9, dpi=300)

# Summary measure of error in e0 ----

# Differences of means
DM_e0 <- bind_rows(lt_HMD, lt_hetfert0b) %>%
...
# Mean of differences
MD_e0 <- bind_rows(lt_HMD, lt_hetfert0b) %>% 
  filter(Year > 1750 & Age == 0) %>% 
  pivot_wider(id_cols = c(Year, sex), names_from = "Source", values_from = "ex") %>% 
  mutate(Error = SOCSIM - HMD) %>% 
  group_by(Year, sex) %>% 
...
ggsave(file="Graphs/HFD_SOCSIM_e0_Error_hetfert0.jpeg", width=17, height=9, dpi=300)
...
bind_rows(TFR_HFCD %>% rename(Estimate = TFR) %>%  mutate(Rate = "TFR"),
          TFR_hetfert0 %>% rename(Estimate = TFR) %>% mutate(Rate = "TFR")) %>% 
  mutate(sex = "female") %>%   
  bind_rows(lt_HMD %>% filter(Age == 0) %>% rename(Estimate = ex) %>% mutate(Rate = "e0"),
            lt_hetfert0b %>% filter(Age == 0) %>% rename(Estimate = ex) %>% mutate(Rate = "e0")) %>% 
  filter(sex == "female") %>%
  mutate(transp = ifelse(Source == "SOCSIM", "0", "1"),
  ...
  theme_graphs()
Summary
ggsave(file="Graphs/Socsim_HFD_HMD2_hetfert0.jpeg", width=17, height=9, dpi=300)