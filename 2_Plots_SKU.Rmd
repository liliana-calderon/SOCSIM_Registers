---
title: "Attempting to replicate the Swedish Kinship Universe plots with SOCSIM data and Excel files"
author: '...'
date: "`r Sys.Date()`"
output:
  github_document:
...
SKU_7 <- SKU %>%
   filter(Fig == "Fig7") %>% 
  pivot_wider(names_from = type, values_from = mean_kin) %>% 
  mutate_at(c(15:19), ~ replace_na(.,0)) %>%  # Replace by 0 to sum
  mutate(registered = (`registered deceased` + `grandmother (mother's side)` + `grandmother (father's side)` + `grandfather (mother's side)` + `grandfather (father's side)`)) %>% 
  # The count of unregistered grandparents may be duplicated
  group_by(IDbirthYear) %>%
  mutate(unregistered = 4 - sum(registered)) %>%
  ungroup() %>%
  mutate_at(c(15:21), ~ na_if(., 0)) %>%
  pivot_longer(cols = c(15:21), 
               names_to = "type", values_to = "mean_kin") %>%
  mutate(type = factor(type,
  ...
  #             color = "grey30", lwd = 0.7) +
    ...
    geom_area(data = SKU_7 %>% 
            ...
            mapping = aes(x = IDbirthYear, y =  mean_kin, fill = type), 
              color = "grey30", lwd = 0.7) +
  geom_vline(xintercept = c(1977), color = "black", lty = 2) +
  scale_fill_manual(values = c("white", #"#FED98E", 
...
Fig8b_socsim <- ggplot() +
  ...
  scale_fill_manual(values =c("#081D58","#253494","#225EA8","#1D91C0","#41B6C4","#7FCDBB","#C7E9B4","#EDF8B1","white","#FFFFD9"),
                    ...
                    labels = c("Grandchildren", "Children", "Nieces/nephews", "Siblings", "Cousins", "Parents", "Aunts/Uncles", "Grandparents", "", "Dead kin")) +
  scale_x_continuous(breaks   = c(seq(1920, 2010,by = 10), 2017), labels = paste(c(seq(1920, 2010,by = 10), 2017), "\n", "(",2018 - c(seq(1920, 2010,by = 10), 2017),")")) +
  theme_bw() +
  theme(panel.grid.major.y = element_line(colour = "grey60", size = 0.3, linetype = 9),
        panel.grid.major.x = element_line(colour = "grey60", size = 0.3, linetype = 9),
